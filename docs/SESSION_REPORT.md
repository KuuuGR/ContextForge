...
- Implemented transcript discovery (metadata only — no transcript download).
- `TranscriptTrack` domain model (`lib/models/transcript_track.dart`):
  - language (reuses `TranscriptLanguage` enum), isAutoGenerated, languageCode, languageName, isTranslatable.
- Extended `YoutubeTranscriptInfo` provider DTO with `languageCode` + `isTranslatable`.
- Implemented `YoutubeExplodeProvider.getAvailableTranscripts`:
  - Uses `yt.videos.closedCaptions.getManifest(videoId)`.
  - Maps `ClosedCaptionTrackInfo` → `YoutubeTranscriptInfo` (language code/name, isAutoGenerated).
  - Injectable `fetchManifest` seam for tests (no network access in tests).
  - External exceptions mapped to domain exceptions (unavailable video, invalid URL, network failure).
- `TranscriptService` (`lib/services/transcript_service.dart`):
  - `getAvailableTranscripts(videoId)` maps DTOs → domain `TranscriptTrack` (ADR-009).
  - `discoverTranscriptTracks(videoId)` throws `TranscriptsUnavailableException` when empty.
- Added `TranscriptsUnavailableException` domain exception.
- Unit tests (`test/transcript_service_test.dart`):
  - mapping, auto-generated transcript, manual transcript, no transcripts, unavailable video, network failure.
- Verified `flutter analyze` (clean) and `flutter test` (145 tests passed).

## Files Created

- `lib/models/transcript_track.dart`
- `lib/services/transcript_service.dart`
- `test/transcript_service_test.dart`

## Files Modified

- `lib/providers/youtube_transcript_info.dart` (languageCode, isTranslatable)
- `lib/providers/youtube_explode_provider.dart` (getAvailableTranscripts + fetchManifest seam)
- `lib/exceptions/youtube_exceptions.dart` (TranscriptsUnavailableException)
- `test/youtube_provider_dto_test.dart` (updated constructors)
- `pubspec.yaml` (0.1.2+1)
- `docs/CHANGELOG.md`, `docs/ROADMAP.md`, `docs/PROJECT_STATE.md`, `docs/SESSION_REPORT.md`

## Known Risks

- `getManifest` may return duplicated tracks per format; the provider maps each track info entry once (deduplication by language+auto not performed yet).
- `isTranslatable` is not exposed by the package's `ClosedCaptionTrackInfo`; defaults to `false`.
- Transcripts disabled/private may surface as `TransientFailureException` → `YoutubeNetworkException` (technically a network failure, semantically "transcripts unavailable").

## Next Phase

Phase 013 — Clipboard Support (status: Next on the roadmap).
...
- `TranscriptService` is the correct home for discovery; it maps provider DTOs → domain models (ADR-009).
- Reuse of `TranscriptLanguage` (polish, polishAuto, english, englishAuto, other) covers pl/en + manual/auto; other languages map to `other` without extending the enum.
- Empty manifest = no transcripts available → `TranscriptsUnavailableException` via `discoverTranscriptTracks`.
- `isTranslatable` defaults to `false` because the package does not expose it.

### Potential Risks

- Deduplication of tracks (per-format duplicates) is not handled; a later phase may add dedup by language+auto.
- YouTube API changes could alter `getManifest` behavior; package must be kept up to date.
- `TransientFailureException` from the package is mapped to `YoutubeNetworkException`; may need finer distinction (e.g., transcripts disabled) if surfaced to users.
</content>
